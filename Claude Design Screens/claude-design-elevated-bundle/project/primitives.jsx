...
function DarkHeader({ lang = 'English', compact = false }) {
  ...
  return (
    <header style={{
      ...
      <Wordmark />
      <nav className="font-sans-ui"
           style={{ display: 'flex', alignItems: 'center', gap: 28,
                    fontSize: 13.5, color: 'oklch(0.78 0.008 265)' }}>
        {items.map((i) => (
      ...
      <div className="font-sans-ui"
           style={{ display: 'flex', alignItems: 'center', gap: 16,
                    fontSize: 13, color: 'oklch(0.78 0.008 265)' }}>
...
// ─── Signed-out / signed-in marketing header ──────────────────
// signedIn prop: { firstName?, email } → renders avatar + pulldown caret
function MarketingHeader({ rtl = false, locale = 'en', signedIn = null }) {
  const signIn = locale === 'he' ? 'התחברות' : locale === 'ar' ? 'تسجيل الدخول' : 'Sign in';
  const initials = signedIn ? (signedIn.firstName || signedIn.email[0]).slice(0, 1).toUpperCase() : '';
  ...
  return (
    <header style={{
      ...
      <Wordmark />
      {signedIn ? (
        <button className="font-sans-ui inline-flex items-center gap-2"
          style={{
