  ...
  try {
    const res = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 90,
        messages: [
          { role: "system", content: "You turn a dictionary word and its meaning into a short ENGLISH description of ONE clear, literal illustration that represents that EXACT meaning, for a children's educational app. Describe a single concrete subject or a simple scene, instantly recognizable. Anchor the illustration on THIS SPECIFIC meaning, not the word's most famous sense: if a word has several meanings, the picture must let a child tell this meaning apart from the others. In particular, when the meaning is a goal, ambition, hope or wish for the future (a 'dream' someone wants to achieve, a plan, an aspiration), show a person AWAKE and ACTIVE imagining or reaching for that goal (for example a child standing and looking up at what they want to become) — never a person sleeping or lying in bed; reserve sleeping imagery only for meanings that are literally about sleep. If the word is itself about letters, vowel marks, punctuation, digits or numbers, DO show those symbols clearly (this is the one case where letters or numbers belong in the picture, e.g. Hebrew letters with vowel points for the word ניקוד). Otherwise avoid written text. Reply with ONLY the description, one sentence, no preamble." },
...
export async function POST(req: NextRequest) {
  try {
    ...
    void recordActivity({ kind: "image", word, lang: uiLangCode, uid: userInfo.userId, plan: userInfo.plan, country: req.headers.get("x-vercel-ip-country"), ua: req.headers.get("user-agent") });
