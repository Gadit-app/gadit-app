...
/**
 * /say — "Say it in ___", pronunciation practice for language learners.
 *
 * Type a sentence in a language you know, pick the language you're
 * learning, and Gadit shows it in that language and reads it aloud in
 * the right accent (via /api/tts, which speaks all 22 languages). Built
 * for a South-African parent whose son is learning Zulu and needs to
 * hear how a sentence is actually pronounced. (Gadi 2026-08-18)
 */
...
export function SayClient() {
  ...
  // Keep the "I'll type in" default aligned with the UI language.
  ...
  // Auto-play the spoken answer once per new result (Clear/Deep only;
  // Basic users still see the text and can tap the speaker for the
  // Web-Speech fallback).
  useEffect(() => {
  ...
  const learnable = LANGUAGES; // any of the 22 can be the target
  ...
  return (
    <div dir={dir} style={{ minHeight: "100dvh", background: "var(--wb-bg, #F7F7F5)", color: "var(--wb-ink, #14181F)" }}>
      ...
      <main style={{ maxWidth: 640, margin: "0 auto", padding: "8px 20px 64px" }}>
        <h1 style={{ fontSize: 30, fontWeight: 800, lineHeight: 1.15, margin: "16px 0 8px", textWrap: "balance" }}>{t.title}</h1>
        <p style={{ fontSize: 16, opacity: 0.7, margin: "0 0 24px" }}>{t.sub}</p>
