...
const word = process.argv[2] || "יד";
const uiLang = process.argv[3] || "he";
...
final.meanings.forEach((m, i) => {
  console.log(`  ${i + 1}: ${m.meaning.slice(0, 70)}`);
  if (m.idioms && m.idioms.length) {
    m.idioms.forEach((id) => console.log(`     - "${id.phrase}" → ${id.meaning}`));
  } else {
    console.log("     (no idioms for this meaning)");
  }
});
console.log("\ngeneralIdioms count:", final.generalIdioms?.length || 0);
(final.generalIdioms || []).forEach((id) => console.log(`  - "${id.phrase}" → ${id.meaning}`));
process.exit(0);
